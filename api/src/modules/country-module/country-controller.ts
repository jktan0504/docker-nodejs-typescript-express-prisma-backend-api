...
import { Country } from './country-prisma-model';
import {
    ICountry,
...
const TABLE_NAME = DataBaseTables.COUNTRIES_TABLE.toUpperCase();
...
export const create = async (
    ...
    try {
        const data = request.body as ICountry;
        data.id = genId();
        const newData = await Country.create(data);
        if (newData) {
            return {
                statusCode: 201,
                data: newData,
...
export const getAll = async (
    ...
    try {
        ...
        if (per_page && current_page) {
            const { data: data, meta: meta } = await Country.getAllPaginate({
                per_page,
                current_page,
            });
        ...
        const query: ICountryQueryOptions = request.query;
        if (query) {
            const result = await Country.getBy(query);
            if (!result) {
                return {
                    statusCode: 404,
                    error: 'data not found',
...
export const update = async (
    ...
    try {
        ...
        const data = request.body as ICountry;
        data.id = id;
        await Country.update(id, data);
        await Country.getById(id);
...
export const bulkCreate = async (
    ...
    try {
        ...
        const data = await Country.bulKCreate(dataRecords as ICountry[]);
