...
const TABLE_NAME = DataBaseTables.MEMBERSHIPS_TABLE.toUpperCase();
...
export const create = async (
    ...
    try {
        const data = request.body as IMembership;
        data.id = genId();
        const newData = await Membership.create(data);
        if (newData) {
            return {
                statusCode: 201,
                data: newData,
...
export const getAll = async (
    ...
    try {
        ...
        if (per_page && current_page) {
            const { data: data, meta: meta } = await Membership.getAllPaginate({
                per_page,
                current_page,
            });
        ...
        const query: IMembershipQueryOptions = request.query;
        if (query) {
            const result = await Membership.getBy(query);
            if (!result) {
                return {
                    statusCode: 404,
                    error: 'data not found',
...
export const getById = async (
    ...
    try {
        ...
        const data = await Membership.getById(id);
...
export const update = async (
    ...
    try {
        ...
        const data = request.body as IMembership;
        data.id = id;
        await Membership.update(id, data);
        await Membership.getById(id);
...
export const bulkCreate = async (
    ...
    try {
        ...
        const data = await Membership.bulKCreate(dataRecords as IMembership[]);
...
export const bulkDestroy = async (
    ...
    try {
        ...
        if (Array.isArray(ids) && ids.length > 0) {
            await Membership.bulKDelete(ids);
            return {
                statusCode: 204,
                data: {},
