...
import { Provider } from './provider-prisma-model';
import {
    IProvider,
...
const TABLE_NAME = DataBaseTables.PROVIDERS_TABLE.toUpperCase();
...
export const create = async (
    ...
    try {
        const data = request.body as IProvider;
        data.id = genId();
        const newData = await Provider.create(data);
        if (newData) {
            return {
                statusCode: 201,
                data: newData,
...
export const getAll = async (
    ...
    try {
        ...
        if (per_page && current_page) {
            const { data: data, meta: meta } = await Provider.getAllPaginate({
                per_page,
                current_page,
            });
        ...
        const query: IProviderQueryOptions = request.query;
        if (query) {
            const result = await Provider.getBy(query);
            if (!result) {
                return {
                    statusCode: 404,
                    error: 'data not found',
...
export const bulkCreate = async (
    ...
    try {
        ...
        const data = await Provider.bulKCreate(dataRecords as IProvider[]);
