...
export class Role {
    ...
    static async getBy(options: IRoleQueryOptions) {
        const where = {};
    ...
    static async bulKCreate(data: Array<IRole>) {
        const providers = data as IRole[];
        return await this.setTable().create({
            data: providers as any,
        });
    }

    static async update(id: string, entityModel: IRole) {
